...
try {
  ...
  const chatId = data.id; // Assuming this is the chat ID, adjust if needed.
  const host = data.host;
  // ...
} catch (err) {
  console.error("Error leyendo data.json:", err);
  process.exit(1); // Salir con un código de error si data.json no se puede leer.
}
...
socket.on('deviceConnected', (data) => {
  ...
  console.log(message);
  bot.sendMessage(chatId, message); // Usando el chatId del archivo JSON
});
